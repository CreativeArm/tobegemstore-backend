...
const BANK_DETAILS = {
  accountName: 'Tobegem Accessories Ltd',
...
router.post('/paystack/initialize', protect, asyncHandler(async (req, res) => {
  ...
  const paystackReq = https.request(options, (paystackRes) => {
    ...
    paystackRes.on('end', () => {
      ...
      if (response.status) {
        ...
        res.status(400).json({ success: false, message: response.message || 'Paystack initialization failed' });
      }
    });
  });
...
router.post('/paystack/verify', protect, asyncHandler(async (req, res) => {
    ...
    paystackRes.on('end', async () => {
      ...
      if (response.status && response.data.status === 'success') {
        ...
        if (order) {
          ...
          order.addTrackingEvent('confirmed', 'Payment received via Paystack');
          await order.save();
        }
...
router.post('/bank-transfer/confirm', protect, asyncHandler(async (req, res) => {
  ...
  order.addTrackingEvent('confirmed', 'Bank transfer submitted, awaiting confirmation');
  await order.save();

  res.json({ success: true, message: 'Bank transfer details submitted. We will confirm within 24 hours.', order });
}));
...
router.post('/paypal/verify', protect, asyncHandler(async (req, res) => {
